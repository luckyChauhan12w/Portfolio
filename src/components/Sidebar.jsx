...
const Sidebar = () => {
  ...
  return (
    <aside
      className={`fixed left-0 top-16 h-[calc(100vh-4rem)] w-50 border-r-2 p-6 z-10 flex flex-col gap-4 transition-colors duration-200 ${
        isDark
      ...
      <nav className="flex flex-col gap-2 mb-6">
        {sections.slice(0, 2).map((section) => (
          <Link
            key={section.name}
            to={section.to}
            className={`flex items-center gap-2 px-3 py-1 text-left transition-colors ${
              isActive(section.to)
                ? isDark
        ...
        >
          <Link
            to="/projects"
            className={`flex items-center gap-2 px-3 py-1 text-left cursor-pointer transition-colors ${
              isProjectActive
                ? isDark
                  ? "underline underline-offset-4 decoration-2 decoration-white"
                  : "underline underline-offset-4 decoration-2 decoration-black"
          ...
          >
            {projectLinks.map((project) => (
              <Link
                key={project.name}
                to={project.to}
                className={`flex items-center gap-2 px-3 py-1 text-left text-sm transition-colors ${
                  isActive(project.to)
                    ? isDark
                      ? "underline underline-offset-4 decoration-2 decoration-white"
                      : "underline underline-offset-4 decoration-2 decoration-black"
